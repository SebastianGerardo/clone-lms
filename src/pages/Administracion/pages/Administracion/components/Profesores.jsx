  ...
  return (
    <div className="flex flex-col place-items-center p-2 min-h-full h-full md:min-w-[50%] xl:min-w-[80%]">
      ...
      <section className="w-full p-2 rounded-xl overflow-y-auto flex flex-col gap-y-4">
            {/* PROFESOR 1 */}
            {profesores.length > 0 && profesores.map((profesor, index) => (
              <div key={profesor.id} className="flex items-center gap-4 justify-between border-b pb-6 border-gray-300">
                <section className='flex gap-3'>
                  ...
                  <div>
                      <h3 className="font-semibold text-[#676767] text-sm">{profesor.nombres}</h3>
                      <p className="text-gray-400 text-xs">{profesor.correo}</p>
                  </div>
                </section>
                <section>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
